/**
 * F10C2 CR1-E — apply migration 216 only on the authorized disposable.
 * Never prints secrets. Never executes 009/010/012/013/112/207/214.
 * Never cleanup. Never reapplies 209–215.
 */
...
function targetInput(env, appEnv) {
  return {
    ...
    commandCategory: 'cr1e-migration-apply',
...
function verificationPassed(row) {
  if (!row) return false
  return Boolean(
    row.has_security_definer_status_rpc
    && row.no_client_update_policy
    && row.anon_cannot_execute
    && row.authenticated_can_execute
    && row.no_207_leak,
  )
}

function verificationPartial(row) {
  if (!row) return false
  if (row.has_security_definer_status_rpc !== true) return false
  return !verificationPassed(row)
}
...
const IMMUTABLE_SNAPSHOT_SQL = `
  SELECT r.id AS run_id,
         ...
         COALESCE(
           r.data_summary #>> '{metrics,dl_mbps_avg}',
           r.data_summary #>> '{scenarios,0,metrics,dl_mbps_avg}'
         ) AS dl_avg,
         COALESCE(
           r.data_summary #>> '{metrics,ul_mbps_avg}',
           r.data_summary #>> '{scenarios,0,metrics,ul_mbps_avg}'
         ) AS ul_avg,
         COALESCE(
           r.gps_summary->>'valid_count',
           r.gps_summary->>'sample_count',
           r.data_summary #>> '{session,gps_points}'
         ) AS gps_valid,
         COALESCE(r.gps_summary->>'invalid_count', '0') AS gps_invalid
...
async function main() {
  ...
  const proof = [
    'F10C2 CR1-E SANITIZED TARGET PROOF',
  ...
  writeEvidence('F10C2_CR1E_Sanitized_Target_Proof.txt', proof.join('\n') + '\n')
  if (reasons.length) {
    throw new Error(`target_rejected: ${reasons.join('; ')}`)
  }

  const existing = listExistingDisposableCr1eApply()
  const full = listPhase4bApplyPlan()
  const applySlugs = existing.map((s) => s.slug)
  assertNo214InApplyList(applySlugs, 'applyCr1eMigrations')
  const leaked = applySlugs.filter((s) => NEVER.has(s) || s.startsWith('207_') || s.startsWith('009_') || s.startsWith('010_') || s.startsWith('214_') || s.startsWith('215_'))
  const unrelated = applySlugs.filter((s) => s !== SLUG)
  console.log('SANITIZED APPLY LIST (existing disposable CR1-E)')
  for (const step of existing) console.log(`  ${step.slug}`)
  writeEvidence(
    'F10C2_CR1E_Apply_List.txt',
    [
      'existing_disposable_cr1e_apply_only:',
      ...applySlugs.map((s) => `  ${s}`),
      ...
      'did_not_include=214,215,209,210,211,212,213',
    ].join('\n') + '\n',
  )
  ...
  if (applySlugs.join(',') !== CR1E_APPLY.join(',') || applySlugs[0] !== SLUG) {
    throw new Error('apply_list_must_be_only_216')
  }

  const step = existing[0]
  if (!fs.existsSync(step.file)) throw new Error('216_forward_missing')
  const verifyPath = path.join(ROOT, 'supabase/drafts/f10c2/phase4b/verification', `${SLUG}.sql`)
  if (!fs.existsSync(verifyPath)) throw new Error('216_verify_missing')
  const forwardText = fs.readFileSync(step.file, 'utf8')
  if (/\b(DROP\s+TABLE|TRUNCATE|DROP\s+DATABASE)\b/i.test(forwardText)) {
    throw new Error('216_forward_contains_destructive_sql')
  }
  if (/\bnsne/i.test(forwardText) || forwardText.includes(WITHDRAWN_TRANSCRIPTION_REF)) {
    throw new Error('216_forward_contains_denied_ref')
  }
  ...
  try {
    ...
    writeEvidence('F10C2_CR1E_Immutable_Snapshots_Before.json', JSON.stringify(before, null, 2))
    ...
    if (alreadyPresent) {
      console.log('VERIFY 216 (already present; skip re-apply)')
    } else {
      if (verificationPartial(preVerify[0])) {
        writeEvidence('F10C2_CR1E_216_Verify_Partial.json', JSON.stringify(preVerify, null, 2))
        throw new Error('216_partial_apply_detected_stop')
      }
      console.log('APPLY 216_cr1e_acceptance_profile_status')
      await sql.unsafe(forwardText)
      status = 'newly_applied'
      console.log('VERIFY 216')
    }

    const verifyRows = alreadyPresent ? preVerify : await sql.unsafe(fs.readFileSync(verifyPath, 'utf8'))
    writeEvidence('F10C2_CR1E_216_Verify.json', JSON.stringify(verifyRows, null, 2).slice(0, 20000))
    if (!verificationPassed(verifyRows[0])) {
      throw new Error('216_verification_failed')
    }

    const after = await sql.unsafe(IMMUTABLE_SNAPSHOT_SQL)
    writeEvidence('F10C2_CR1E_Immutable_Snapshots_After_Apply.json', JSON.stringify(after, null, 2))
    const beforeKey = JSON.stringify(before)
    const afterKey = JSON.stringify(after)
    if (beforeKey !== afterKey) {
      throw new Error('immutable_snapshots_changed_during_216')
    }

    writeEvidence(
      'F10C2_CR1E_Applied_Migrations.txt',
      [
        `migration_id=216`,
        `slug=${SLUG}`,
        `status=${status}`,
        ...
        'did_not_reapply=209,210,211,212,213,215',
        `project_ref=${AUTHORIZED_DISPOSABLE_PROJECT_REF}`,
        'immutable_snapshots_unchanged=yes',
      ].join('\n') + '\n',
    )
    console.log(`CR1-E 216 ${status} and verified`)
...
main().catch((error) => {
  ...
  console.error(`APPLY_CR1E_FAILED: ${message}`)
  writeEvidence('F10C2_CR1E_Apply_Failure.txt', `message=${message}\n`)
  process.exit(1)
})
