...
const GPS_PERMISSION_REQUIRED_MESSAGE = "Location permission is required for live task GPS tracking.";
const GPS_PERMISSION_ANDROID_STEPS =
  "Android Settings > Apps > BabyDragon > Permissions > Location > Allow while using.";
const GPS_PERMISSION_OR_SIGNAL_MESSAGE =
  `${GPS_PERMISSION_REQUIRED_MESSAGE} ${GPS_PERMISSION_ANDROID_STEPS} If permission is already allowed, move where GPS can lock.`;
...
export default function MobileApp() {
  ...
  async function refreshGpsNow(options = {}) {
    ...
    setGpsStatusMessage(GPS_PERMISSION_OR_SIGNAL_MESSAGE);
    if (!silent) setGpsChecking(false);
    return null;
  }
  ...
  return (
    <main className="bd-mobile-screen">
      ...
      <nav className="bd-mobile-bottom-nav">
