...
// BabyDragon / NetField-360 - Cell File Parser V1

export const CELL_FILE_SUPPORTED_EXTENSIONS = [".txt", ".csv"];
...
export function parseCellFileText(text, options = {}) {
  ...
  if (!text || !String(text).trim()) {
    return emptyResult({ fileName, market, technology: defaultTechnology, error: "Cell file is empty." });
  }
  ...
  if (lines.length < 2) {
    return emptyResult({ fileName, market, technology: defaultTechnology, error: "Cell file must contain a header row and at least one data row." });
  }

  const delimiter = detectDelimiter(lines[0]);
  const rawHeaders = splitLine(lines[0], delimiter);
  const mappedHeaders = mapHeaders(rawHeaders);
  const missingFields = CELL_FILE_REQUIRED_FIELDS.filter((field) => !mappedHeaders.includes(field));
  ...
  if (missingFields.length) {
    return emptyResult({
      ...
      warnings: [`Detected headers: ${rawHeaders.join(", ")}`, "Expected at minimum: SITE, LAT, LON"],
    });
  }
  ...
  for (let i = 1; i < lines.length; i += 1) {
    ...
    const siteKey = buildSiteKey({ market, siteName, lat, lon, technology });
    ...
    sectors.push({
      ...
      lat,
      lon,
      azimuth: toNullableNumber(row.azimuth),
      antenna_bw: toNullableNumber(row.antenna_bw),
  ...
  const detectedTechnologies = uniqueClean(sectors.map((sector) => sector.technology).filter(Boolean));
  const technology = defaultTechnology || (detectedTechnologies.length === 1 ? detectedTechnologies[0] : "Mixed");

  if (!sectors.length && !errors.length) warnings.push("No valid sector records found.");
...
export function buildSiteIdMap(insertedSites = [], parsedSites = []) {
  ...
  parsedSites.forEach((parsedSite) => {
    ...
    if (match?.id) map[parsedSite.temp_key] = match.id;
  });
...
function emptyResult({ fileName, market, technology, error, warnings = [] }) {
  return {
    ok: false,
    file: { file_name: fileName, market: market || null, technology: technology || null, record_count: 0 },
    delimiter: null,
    headers: [],
    mapped_headers: [],
    sites: [],
    sectors: [],
    stats: { total_lines: 0, total_sites: 0, total_sectors: 0, technologies: [] },
    errors: error ? [error] : [],
    warnings,
  };
}
...
function detectDelimiter(headerLine) {
  const candidates = ["\t", ",", ";", "|"];
  let best = { value: "whitespace", count: 0 };

  candidates.forEach((candidate) => {
    const count = countOccurrences(headerLine, candidate);
...
function splitLine(line, delimiter) {
  if (delimiter === "whitespace") {
    return String(line).trim().split(/\s+/).map(cleanText);
  }
...
function mapHeaders(rawHeaders) {
  return rawHeaders.map((header) => {
    ...
    for (const [standardName, aliases] of Object.entries(HEADER_ALIASES)) {
      ...
      if (normalizedAliases.includes(normalizedHeader)) return standardName;
    }
...
function buildRowObject(rawHeaders, mappedHeaders, values) {
  ...
  rawHeaders.forEach((header, index) => {
    ...
    raw[rawHeader] = value;
    if (!row[mappedHeader]) row[mappedHeader] = value;
  });
...
function buildSiteKey({ market, siteName, lat, lon, technology }) {
  return [cleanText(market).toLowerCase(), cleanText(siteName).toLowerCase(), Number(lat).toFixed(6), Number(lon).toFixed(6), cleanText(technology).toLowerCase()].join("|");
}
